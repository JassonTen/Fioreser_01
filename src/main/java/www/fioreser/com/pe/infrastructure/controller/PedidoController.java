...
public class PedidoController {
    ...
    public String showSumaryOrder(Model model){
       ...
       // model.addAttribute("nombre", httpSession.getAttribute("name").toString());
        return "cliente/summaryorder";
    }
    @GetMapping("/create-pedido")
    public String createPedido(RedirectAttributes attributes) {
        UsuarioEntity user = usuarioService.findById(1);
        PedidoEntity pedido = new PedidoEntity();
        pedido.setFechaCreada(LocalDateTime.now());
        pedido.setStatus("Proceso");
        pedido.setUsuarioEntity(user);
        pedido.setTotal(carritoService.getTotalCart().toString());
        log.info("order : {}", pedido);
        ...
        for (ItemCarrito itemCart : carritoService.getItemCarts()) {
            pedidoProduct.add(new PedidoProductEntity(productService.getProductById(itemCart.getIdPrC()),itemCart.getCantidad(),pedido));
        }
